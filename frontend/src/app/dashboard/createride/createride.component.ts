...
export class CreaterideComponent {
  ...
  constructor(private http: HttpClient, private fb: FormBuilder, private _createride: CreaterideService,
    private toaster: ToastrService, private _city: CityService, private cdr: ChangeDetectorRef, private _setting: SettingService, private dialog: MatDialog) { }
  ...
  //--------------- TO GET NO. OF STOPS SET BY ADMIN------------------
  getNumberOfStops() {
      ...
      next: (user: any) => {
        ...
        this._city.getcity().subscribe({
          next: (cities: any) => {
            ...
            this.polygonObjects = this.polygons.map(function (polygonCoordinates: any) {
        ...
        geocoder.geocode({ address: input.value }, (results: any, status: any) => {
          if (status === 'OK') {
            ...
            for (var i = 0; i < this.polygonObjects.length; i++) {
              if (google.maps.geometry.poly.containsLocation(location, this.polygonObjects[i])) {
                this.cityIndex = i;
                this.isInZone = true
    ...
    this.directionsService?.route(
      {
        ...
        waypoints: this.waypoints.map((waypoint: any) => ({ location: waypoint })),
  ...
  calculateFare(vehiclePricing: any, cityIndex: any) {
    ...
    let estimatePrice =
      ...
      ((this.totalDistance - baseDistance) * pDistance) + (basePrice) + (this.totalTime * pTime)
  ...
  onBookRide() {
    ...
    this._createride.addRide(this.rideData).subscribe({
      next: (res) => {
        ...
        const dialogRef = this.dialog.open(SuccessDialogComponent, { width: "600px", data: { title: "Ride Booked", content: "Ride booked successfully!" } });
        this.rideForm.reset();
        this.rideForm.patchValue({
          serviceType: ''
        });
